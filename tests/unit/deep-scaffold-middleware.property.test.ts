...
const FIXTURE_NAMES = [
  ...
  "without-matcher",
  "recognized-matcher",
  "unrecognized-matcher",
  "already-present",
];

describe("applyMiddlewareScaffold property tests", () => {
  ...
  it("running twice makes zero additional changes for every fixture", async () => {
    await fc.assert(
      fc.asyncProperty(fc.constantFrom(...FIXTURE_NAMES), async (fixtureName) => {
        ...
        const first = await applyMiddlewareScaffold({ projectRoot, projectType: "nextjs", dryRun: false });
        const afterFirst = await readFileTree(projectRoot);

        const second = await applyMiddlewareScaffold({ projectRoot, projectType: "nextjs", dryRun: false });
  ...
  it("dry-run never writes, regardless of what the computed diff says would change", async () => {
    await fc.assert(
      fc.asyncProperty(fc.constantFrom(...FIXTURE_NAMES), async (fixtureName) => {
        ...
        await applyMiddlewareScaffold({ projectRoot, projectType: "nextjs", dryRun: true });
